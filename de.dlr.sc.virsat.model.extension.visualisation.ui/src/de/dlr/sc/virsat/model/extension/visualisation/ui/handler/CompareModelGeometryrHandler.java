...
import de.dlr.sc.virsat.model.extension.visualisation.ui.dialogs.CompareModelDialog;
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			if (object instanceof VirSatProjectResource) {
				...
				CompareModelDialog dialog = new CompareModelDialog(shell, vsBaseProject);
				
				if (dialog.open() == Window.OK) {
					VirSatProjectResource vsCompareProject = dialog.getComparisonProjectResource();				
					if (vsCompareProject != null) {
						new ProgressMonitorDialog(shell).run(true, false, (pm) -> {
							try {
								...
								deltaModelFile = CreateDeltaModelFolderHandler.getNewComparisonFile(vsBaseProject, CreateDeltaModelFolderHandler.VIS_DELTA_MODEL_GEO_FILE_EXT);
								...
								Activator.getDefault().getLog().log(new Status(Status.ERROR, Activator.getPluginId(), "CompareModelGeometryrHandler: Failed to create DeltaModel ", e));				
							}
						});
					}
