 ...
 * 
 */
public class UpperLimitVerificationStepTest extends AConceptProjectTestCase {
	...
	public class TestUpperLimitRequirementsVerification extends UpperLimitVerification {
		boolean statusCompliant = false;
		boolean statusOpen = false;
		boolean statusNonCompliant = false;
		boolean statusPartlyCompliant = false;
		
		public TestUpperLimitRequirementsVerification() {
	...
	public void testRunCustomVerificationNoTrace() {
		TestUpperLimitRequirementsVerification testObject = new TestUpperLimitRequirementsVerification();
	...
	public void testRunCustomVerificationCompliant() {
		...
		TestUpperLimitRequirementsVerification testVerification = new TestUpperLimitRequirementsVerification();
	...
	public void testRunCustomVerificationTooHigh() {
		...
		// The upper limit verification element is used as random element with a value property for testing 
		// (-> not actually used as verification here)
		LowerLimitVerification someValueElement = new LowerLimitVerification(conceptRequirements);
		someValueElement.setLowerBound(VALUE_TOO_HIGH);
		requirementObject.getTrace().getTarget().add(someValueElement);
		...
		testVerification.setUpperBound(UPPER_LIMIT);
		testVerification.setStatus(IVerification.STATUS_Open_NAME);
		ATypeDefinition propertyToBeVerified = someValueElement.getLowerBoundBean().getATypeInstance().getType();
		testVerification.setElementToBeVerified(propertyToBeVerified);
