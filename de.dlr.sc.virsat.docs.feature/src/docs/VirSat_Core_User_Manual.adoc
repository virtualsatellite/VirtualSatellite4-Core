...
NOTE: The Excel-file contains _Parts_, which were exported from the _Product Tree_, and _Products_,
|exported from the _Configuration_ or _ProductTree_ Tree_. So _Parts_ contain information about the elements,
|e.g. your Cost, Margin, CostWithMargin and CostMargin
|image:chapterModeling/subsectionCost/export/04_output.PNG[Excel output file]
// 
NOTE: The round-trip engineering is driven from the Virtual Satellite side, 
|so that Virtual Satellite can execute all CRUD (Create, Read, Update, Delete) operations, 
|while an external CAD software can only Read and Update existing components.
...
|{counter:step}.
|You can now modify the generated Excel file, e.g. 
change Name, Cost o the CostType of the `CostEquipment`. 
Afterwards it can be reimported to update the Virtual Satellite Model.
|image:chapterModeling/subsectionCost/import/00_changed_output.PNG[Changed Excel output file]

|{counter:step}.
|Then select the Cad Import Wizard.
|image:chapterModeling/subsectionCost/import/01_Import.png[Import]

|{counter:step}.
|Similar to exporting you open the _Import_ Wizard.
|image:chapterModeling/subsectionCost/import/02_Cost_Import_Wizard.PNG[Import Wizard]

|{counter:step}.
|Then select the Cad Import Wizard.
In the Wizard, select the Tree to load the Excel file into and the destination of the file.
|image:chapterModeling/subsectionCost/import/03_tree_and_destination_selection.PNG[Tree and destination selection]

|{counter:step}.
|Finally you can open the `CostMaterial` 's CostEquipment to see if the property has been updated.
|image:chapterModeling\subsectionCost\import\04_model_changes.PNG[Imported Model changes]
