...
====== Import from ReqIF-files from DOORs
...
|{counter:step}.
|Add specifications to ReqIF file which are shall be exported
...
|{counter:step}.
|Select `ReqIF Requirements Import` form the `Virtual Satellite` category.
|image:chapterModeling/subsectionRequirements/reqif/ReqIfImportType.png[ReqIF Import]

|{counter:step}.
|Select a ReqIF file and a container for the new import configuration (Select a RequirementsConfigurationCollection in the tree)
|image:chapterModeling/subsectionRequirements/reqif/SelectFileAndConfigContainer.png[Select ReqIF file]

|{counter:step}.
|Select the requirement specifications from the ReqIF file which shall be imported. For each of them select a container tree element in which a new specification shall be created.
