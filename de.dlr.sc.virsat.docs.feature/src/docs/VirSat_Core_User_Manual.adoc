...
•	Activate your required concepts, such as PS or CEF and requirements 

image:chapterModeling/subsectionRequirements/ActivateConcepts.png[Activate required concepts]
...
* This class is used to create requirement types: these types specify the attributes each of its requirements has
...
|{counter:step}.
|Go to the model location where you want to add requirements (requirements can also be split to several elements within a model). 
...
|{counter:step}.
|When a new requirement should be added, the editor asks of which type it should be: 

* You can now choose between the types you create in the configuration element of your project. 
The left section of the dialog allows to customize the container location of the configuration 
element, the right side show all applicable types in the currently selected tree element.
...
====== Map CSV requirements columns to existing type 
