...
The four different trees are called: *Product Tree*, *Configuration Tree*, *Assembly Tree* and *Storage*.
...
NOTE: The _Assembly Tree_ also combines information from the storage.
E.g. an equipment in the storage may have a unique serial number.
Once plugged into the _Assembly Tree_ the serial number will be finally overridden with the one from the equipment in the _Storage_
...
TIP: The _Storage_ is usually the place to save information such as real life measurements or serial numbers.

NOTE: E.g. in the storage we create three instances of the star trackers which inherit from the one defined in the _Product Tree_.
Now we get three real star trackers delivered from the supplier.
We assign the serial numbers to their representation in the storage.
From the storage we assign two of them into the _Assembly Tree_.
...
Therefore two instances of the reaction wheel are created in the _Storage_ called _RW_A_ and _RW_B_. 
...
Hence the calibration curves are now adjusted in the _Storage_.
...
Therefore the _RW1_ in the _Assembly Tree_ has to additionally inherit from the _RW_A_ in the _Storage_.
...
Then, five star trackers will be modeled in the storage, with actual weighed mass.
...
At the moment we don't yet need the _Storage_.
...
NOTE: Create an example project as you have seen in the chapter "<<First Steps>>" before you continue with this tutorial.
...
In certain cases it is necessary to start Virtual Satellite as a different user.
This can be done by changing the `VirSat4_Core.ini` file.
Open the file in a text editor of your choice and add the parameter `-forceUser <name>` as given in the example below.
...
case you are not allowed to do this. Gaining super user rights can be done by changing the `VirSat4_Core.ini` file.
Open the file in a text editor of your choice and add the parameter `-superUser` as given in the example below.
...
The OSGI console of eclipse can be very helpful in case things does not work as expected.
