...
===== Add interface end to the model
Virtual Satellite enables the user to add interfaces to the system model.
The functional electrical architecture is added to the register by opening it with the right mouse and click on Add register in the table chose functional electrical architecture.S
An interface can be added to a configuration tree by using the right mouse and navigating to Functional Electrical Architecture/Add Interface.
The interface end and start point can be set as properties of the interface
image:chapterModeling/subsectionFEA/InterfaceEnd.PNG[InterfaceEnd]

Additional to the Interface end the interface is supposed to be assigned to an interface type.
A type can be selected by clicking on Drill-Down.
The types need to be defined as child elements of an InterfaceTypeCollection.
The InterfaceTypeCollection is created by clicking with the right mouse on the Repository and select Functional Electrical Architecture\Add InterfaceTypeCollection.
image:chapterModeling/subsectionFEA/InterfaceType.png[InterfaceTypeCollection]
A type can be created by clicking with the right mouse on the InterfaceTypeCollection and selecting Functional\Electrical Architecture\Add InterfaceType
...
|Image
|{counter:s}.
|Add the concept Mechanical Engineering to the repository.
|image:chapterModeling/subsectionFEA/AddMechanicalEngineering.PNG[ME]
...
|{counter:s}.
|Choose VirSat/Virsat Diagram.
|image:chapterModeling/subsectionFEA/Diagram.PNG[NE]

|{counter:s}.
|Choose interfaces in the Drop Down menu.
|

|{counter:s}.
|Drag and Drop the Element Configuration in the Window
|

|{counter:s}.
|The Interface can be used in the system diagram to connect element configurations by drag and dropping the interface to their interface end.
