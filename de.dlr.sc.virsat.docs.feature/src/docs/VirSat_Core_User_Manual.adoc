...
Mathematical dependencies between numeric properties are described by Equations.
...
They can be freely edited using the Equation Editor.
...
to include an additional top-level system margin and ignore the mass of a specified subsystem.
...
|image:chapterAdvancedModeling/OpenRepository.png[Opening the repository editor]
