...
The thermal concept allows the user to add thermal properties to the system model. Ultimately, these properties can be used as the basis for a workflow to perform thermal analyses with the FEM tool CalculiX. This is done by exporting the 3D Visualisation and the thermal properties, processing them utilizing FreeCAD as preprocessing tool. CalculiX then reads the input files, creates, and solves, the equation system. Finally, the thermal concept also allows to import some aspects of the result to the system model.
...
|{counter:step}.
|Accordingly, add a "Thermal Data", and then a "ThermalElementParameters" (TEP might be an own category later) element for each of the components. This element is extremely important for setting up the rest of the model and for the simulation itself, so make sure all desired parts have such an element. Fill this element with the required information comprising the initialTemperature (only relevant for dynamic simulations), the power balance of the part (meaning the dissipated heat), and finally reference the desired material for the part.
|

|{counter:step}.
|If not already present anyways, create a "ThermalControl" (or similar name) subsystem in the configuration tree. Even if not used for physical components. In this subsystem create a "ThermalAnalysis" element. This will host a lot of general, analysis related information. Make sure there is only one of these elements in the tree.
|

|{counter:step}.
|In the "ThermalAnalysis" element, create a "ThermalContacts" element. This is where it gets slightly complex. Each contact between two physical elements (check in the visualization) must be considered to obtain a valid simulation result, as thermal contacts usually conduct heat quite effective. In the "ThermalContacts" element, create one "ThermalInterfaceList" and one "ThermalPortList" element. 
|

|{counter:step}.
|For each contact that exists between two components, two dedicated "ThermalPorts" must be created (it is very important that each thermal port is assigned to exactly one thermal contact, ---TBC---). Both thermal ports are assigned to the respective contact partner component by the reference property. This links to the "ThermalElementParameter" of a desired element. To keep track of all thermal ports it makes sense to name them "TP:<NameOfThePart>".
|

|{counter:step}.
|When the thermal ports are created and assigned to components, the thermal interfaces are modeled. For this, create one "ThermalInterface" element for each contact. In this element, two references and two values are set. The references are linked to the involved thermal ports (remark: the first thermal port is treated as the "master" contact in the following. This is relevant if the two components are overlapping, either on purpose or accidentally. In that case the FreeCAD processing script will cut the overlapping volume from the "slave" component, whereas the "master" component will not be touched.)
|
