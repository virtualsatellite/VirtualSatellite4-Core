...
The 3D Visualisation can be ex- and imported into CAD (computer-aided design)-tools using the JSON (JavaScript Object Notation) format.
This subsection describes the workflow on the Virtual Satellite side of the corresponding Roundtrip Engineering. 
...
|{counter:step}.
|You start with a simple _Repository_ similar as above, containing a _Product_, _Configuration_ and _Assembly Tree_, with a visualization added to the `ST2`.
...
|{counter:step}.
|The _Export_ Wizard opens, in which you select the _Cad Export Wizard_.
...
You also have to provide the destination of the export file.

|image:chapterModeling/subsectionCAD/export/03_tree_and_destination_selection.PNG[Tree and destination selection]

|{counter:step}.
|Finishing the dialog now exports all elements of the selected _Tree_, that contain visualisations, into the JSON output file.
As you can see only the `ST2` got exported, because `ST1` did not contain a visualisation. 
...
NOTE: The Roundtrip Engineering is driven from the Virtual Satellite side, so that Virtual Satellite can execute all CRUD (Create, Replace, Update, Delete) operations, while an external CAD software can only Replace and Update.

NOTE: For the purpose of mapping each element has a corresponding UUID (Universally Unique Identifier).
...
|{counter:step}.
|In the Wizard now select the _Tree_ to load the JSON file into and the destination of that file.
|image:chapterModeling/subsectionCAD/import/03_tree_and_destination_selection.PNG[Tree and destination selection]

|{counter:step}.
|Finally you can open the `ST2` 's Visualisation to check that it's _positionX_ property updated.
