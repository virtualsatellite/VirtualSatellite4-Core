...
|{counter:steps}.
|Start with opening a new perspective by clicking the "window" button in your menu as shown in Figure "Open Perspective".
...
After that you need to visit www.github.com and choose the repository you want to clone. +
...
|{counter:steps}.
|Now you have to open the folder "Working Tree" and scroll down until you find the folder "de.dlr.sc.virsat.target". +
...
|{counter:steps}.
|After that you need to switch from the Git perspective to the Plug-in development perspective in the upper right corner. +
Then take a look at the project explorer in the upper left corner to import a project. +
Select the option "Git" -> "Projects from Git" and continue.
|image::chapterIdeInstallation/subsectionGitIdeSetup/12_ImportProjectsGit.PNG[Import Projects from Git]

|{counter:steps}.
|In the next step you have to select a location of your Git repositories. +
Choose "Existing local repository" and select "VirtualSatellite4-Core [development]- [...] \.git" and continue. +
Next, choose the first option "Import existing Eclipse projects" and go on to the window, where all projects are selected. +
When you have finished the process, you see all projects listed in your Project Explorer on the left side.
|image::chapterIdeInstallation/subsectionGitIdeSetup/15_ImportEclipse.PNG[Import existing Eclipse projects]
