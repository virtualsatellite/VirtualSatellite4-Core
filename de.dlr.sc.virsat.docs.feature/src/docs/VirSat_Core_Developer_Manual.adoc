...
Start by opening a perspective by clicking the "window" button in your menu as shown in Figure "Open Perspective".

.Open perspective
[#ExternalLibraryBinaryBuild]
image::subsectionGitIdeSetup/2_Open perspective.png[Open perspective]

Select the option "Git" and in the next window choose "clone URl" and go on.
Next, you go to github.com and choose the repository you want to clone. 
Click the green button in the upper right corner, which says "clone or download" and copy the link shown there.

.Clone Git URL
[#ExternalLibraryBinaryBuild]
image::subsectionGitIdeSetup/2_Clone Git URL.png[Clone Git URL]
Insert the link into the free field "URl" and authenticate yourself in the section beneath. 
...
Choose a directory as a local destination for VirtualSatellite4-Core and finish the process. 

Hence you select the branch "development" and double click to check out the branch. 

.Check out development branch
[#ExternalLibraryBinaryBuild]
image::subsectionGitIdeSetup/8_Checkout branch.png[Check out development branch]

Now you switch to the node "Working Tree" and scroll down until you see the folder "de.dlr.sc.virsat.target", open it and then select "virsat.target".
Wait until the progress is loaded fully and then set it as your active target platform. The button is in the upper right corner.


.Open virsat.target
[#ExternalLibraryBinaryBuild]
image::subsectionGitIdeSetup/10_Open Virsat Target.png[Open Virsat Target]


Next, you need to switch from the Git perspective to Plug-in development in the upper right corner.
Then take a look at the Project Explorer in the upper left corner to import a project.
Select the option "Git" -> "Projects from Git" and go on.

.Import Projects from Git
[#ExternalLibraryBinaryBuild]
image::subsectionGitIdeSetup/12_Import projects Git.png[Import Projects from Git]

In the next step choose "Existing local repository" and select "VirtualSatellite4-Core [development]- [...] \.git and continue.
Next, choose the first option "Import existing Eclipse projects" and go on to the window, where all projects are selected. 
When you finish the process, you see all projects listed in your Project Explorer on the left side.
You have succesfully set up your Git IDE!
