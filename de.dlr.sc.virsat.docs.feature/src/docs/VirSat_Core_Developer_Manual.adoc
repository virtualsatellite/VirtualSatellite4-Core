...
isVerification::
Defines that the annotated Category Assignment is a verification element that can be added to requirements. A verification category has specific UI and its, to be implemented, verification logic is automatically executed from the requirements verification engine.
The verification implementation can be added into the generated bean's function:
----
@Override
...
* _verification_: 
A verification category that can be used to verify requirements targeting this property. Verification categories can be imported from the requirements concept (UpperLimitVerification...) or be implemented by adding the _isVerification_ to categories. This keyword will lead to additional UI in the requirements Virtual Satellite editor. 
...
<classpath>
	<classpathentry kind="con" path="org.eclipse.jd...auncher.StandardVMType/JavaSE-11"/>
...
=== Loading multiple Concepts in test cases

To test multiple Concepts in one Testcase or Concepts that depend on other Concepts the `AConceptProjectTestCase` class can be used for this.
An EditingDomain and a Repository is necessary to load the concepts. 
The methods of `AConceptProjectTestCase` can be used for this.
The `ActiveConceptHelper` is recommended for getting the loaded Concepts and use them to initialize the Concept objects.

Explanation of the example below:

To initialize the test case `super.setUp()` and `addEditingDomainAndRepository(true)` needs to be called. Afterwards all concepts can be loaded with `+executeAsCommand(() -> loadConceptAndInstallToRepository(CONCEPT_ID))+`. The recommended `ActiveConceptHelper` can be initialized through the `repository` and all concepts can be fetched that are needed to initialize objects. 
An optional `ConfigurationTree` is added to the repository in which the Concept objects can be added.

.Example of a multi concept test
[source,java,#MultiConceptTestExample]
----
public class MultiConceptTestExample extends AConceptProjectTestCase { 
	
	private static final String CONCEPT_ID_DEP = "de.dlr.sc.virsat.model.extension.dependency";
	private static final String CONCEPT_ID_TEST = "de.dlr.sc.virsat.model.extension.testconcept";
	
	protected Concept conceptDependency;
	...
	public void setUp() throws CoreException {
		...
		// CONCEPT_ID_CORE is definded in AConceptProjectTestCase.java. All dependencies have to be loaded first
		executeAsCommand(() -> loadConceptAndInstallToRepository(CONCEPT_ID_CORE));		 
		executeAsCommand(() -> loadConceptAndInstallToRepository(CONCEPT_ID_DEP));		 
		executeAsCommand(() -> loadConceptAndInstallToRepository(CONCEPT_ID_TEST));
		
		acHelper = new ActiveConceptHelper(repository);
		conceptDependency = acHelper.getConcept(CONCEPT_ID_DEP);
		conceptTest = acHelper.getConcept(CONCEPT_ID_TEST);
				
		/*
		 * Initialize your Concept objects here
		 */ 
		
		ConfigurationTree ct = new ConfigurationTree(conceptPS);
		ct.add(/* concept object */);
		executeAsCommand(() -> repository.getRootEntities().add(ct.getStructuralElementInstance()));

		// rs is defined in AProjectTestCase.java
		editingDomain.getCommandStack().execute(new CreateSeiResourceAndFileCommand(rs, ct.getStructuralElementInstance()));		
	}
		
	@Test
	public void testAll() {
		// your JUnit tests here
	}
}
----
