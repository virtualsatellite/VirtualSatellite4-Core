...
isVerification::
Defines that the annotated Category Assignment is a verification element that can be added to requirements. 
A verification category has specific UI and its, to be implemented, verification logic is automatically executed from the requirements verification engine.
...
* _verification_: 
A verification category that can be used to verify requirements targeting this property. 
Verification categories can be imported from the requirements concept (UpperLimitVerification...) or be implemented by adding the _isVerification_ to categories. 
This keyword will lead to additional UI in the requirements Virtual Satellite editor. 
