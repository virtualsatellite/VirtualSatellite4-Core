...
public class CadExporterThermal {
		...
		try (Writer output = new FileWriter(materialFile)) {
			...
			for (IBeanStructuralElementInstance ec : ecs) {
				...
				if ((td != null) && (visShape != null)) {
					...
					double heatCapacity = tep.getPredefinedMaterial().getHeatCapacityBean().getValueToBaseUnit()
							* 1000000;
	...
	public void writeCadBoundaryConditionsInput(String path) throws IOException {
		for (IBeanStructuralElementInstance ec : ecs) {
			...
			if ((td != null) && (visShape != null)) {
				...
				try (Writer ehfOutput = new FileWriter(ehfFile); Writer bcOutput = new FileWriter(bcFile)) {
	...
	public void writeCadContacsInput(String path) throws IOException {
		...
		try (Writer output = new FileWriter(addContactFile);
	...
	public void writeCadMeshSizesInput(String path) throws IOException {
		...
		try (Writer meshSizeOutput = new FileWriter(meshSizeFile)) {
				...
				if ((td != null) && (visShape != null)) {
					...
					for (ComponentMeshSize cms : cmss) {
						if (cms.getMeshComponent().getParent().getName().equals(ec.getName())) {
							// CHECKSTYLE:OFF
							double maximumCharacteristicMeshLength = cms.getMaximumCharacteristicMeshLengthBean()
									.getValueToBaseUnit() * 1000;
							// CHECKSTYLE:ON
